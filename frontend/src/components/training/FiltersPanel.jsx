...
export default function FiltersPanel({
  ...
  return (
    ...
    >
      {isSidebar && (
        <div className="font-bold text-sm tracking-wide mb-4 text-center">FILTER</div>
      )}
      {(showSearch || showSort) && (
          ...
          {showSearch && (
            <div className={isSidebar ? '' : 'flex-1'}>
              <input
                ...
                className="w-full h-10 px-4 rounded-lg border border-neutral-200 bg-white text-sm outline-none focus:ring-2 focus:ring-[#DAB619]/40 focus:border-[#DAB619] transition"
              />
            </div>
          )}
          {showSort && (
