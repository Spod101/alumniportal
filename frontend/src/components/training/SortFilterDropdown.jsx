...
function SortFilterDropdown({ value, onChange, placeholder = 'Sort by' }) {
  ...
  return (
    <div className="relative inline-block text-left w-48 min-w-[160px]" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-full bg-white rounded-lg shadow-xl border border-gray-200 z-50 overflow-hidden text-sm font-medium text-gray-900">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
